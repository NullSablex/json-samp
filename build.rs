...
fn generate_inc() {
    ...
    let template_path = "include/json_samp.inc.in";
    let output_path = "include/json_samp.inc";

    // No `cargo:rerun-if-changed` directives: build.rs runs on every build
    // so the .inc tracks the current Cargo version and template without
    // manual intervention. The write below is idempotent — it only touches
    // disk when the rendered output actually differs from the file on disk,
    // so the always-run policy does not churn timestamps.
